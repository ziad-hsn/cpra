---
title: Candidate · Capacity and diagnostics
description: Candidate · Measure CPRa queue demand, service time, worker limits and target saturation without assuming unverified throughput guarantees.
...
./bin/cpractl get slo -o json
./bin/cpractl get state -o json
./bin/cpractl metrics
~~~
...
Worker limits, queue capacity, snapshot interval, and sizing policy are programmatic controller or worker-pool settings. They are not arbitrary fields in the monitor YAML. The separate runtime file exposes the documented storage/history/SLO settings; use the [runtime reference](../reference/runtime-config.md). The server exposes only the [documented flags](../reference/cli.md).
