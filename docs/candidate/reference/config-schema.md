---
title: Candidate · Monitor configuration
description: Candidate · Configure CPRa YAML manifests, check thresholds, alert groups, maintenance windows and recovery targets.
...
Each endpoint has its own committed action and outcome. A successful endpoint does not complete an unfinished endpoint; confirmed retryable rejections receive at most three attempts for that endpoint, and ambiguous outcomes remain held. `dispatch` defaults to true for a declared rule; set it to false to suppress that rule.
...
`-yaml` or the `-config` manifest alias. Default storage is single-node Raft in the platform user state directory; `cpractl local paths` reports it. A legacy `./cpra-data` requires explicit selection or a stopped migration. `-data-dir` overrides `storage.directory`, which overrides the platform default. Explicit `storage: {mode: memory}` selects disposable state.
