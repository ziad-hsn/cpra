---
title: Candidate · Release engineering
description: Candidate · Release engineering for the reviewed CPRa source; see the version and availability notice.
...
## Documentation source binding

This public site covers multiple identified sources. The candidate release workflow still requires `_sources/source-version.json.source_commit` to equal the exact application being released. A release must select a documentation ref with that exact binding and reviewed candidate content. Merely listing a candidate in `reviewed_sources` does not satisfy or bypass that release gate.
