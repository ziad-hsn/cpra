---
title: SDK candidate · Disable an AWS target's existing monitor
description: SDK candidate · Disable an AWS target's existing monitor for the reviewed CPRa source; see the version and availability notice.
cpra_scope: sdk
---
...
The local demo works now. The configured mode requires CPRa's planned v2 management server; the current CPRa server implements read-only v1. The example and its tests exercise HTTP contracts and local AWS fixtures. They do not prove delivery in an AWS account.

## Run the local lesson

Start in `examples/sdk`, using the source workspace described in [the examples guide](index.md):

```sh
go run ./aws-deregister -demo
```
...
Read [demo.go](source/examples/sdk/aws-deregister/demo.go.txt) first. It constructs a client, creates one monitor, and feeds an event to the same processor that configured SQS mode uses. Then read [processor.go](source/examples/sdk/aws-deregister/processor.go.txt), where the decision to disable is made.
...
| --- | --- |
| [main.go](source/examples/sdk/aws-deregister/main.go.txt), `run` | Reads flags and mapping, constructs the CPRa client, verifies AWS account identity, and chooses SQS or one-event input. |
| [events.go](source/examples/sdk/aws-deregister/events.go.txt), `decodeEvent` and `event.validate` | Decodes and validates the event before it can select a monitor. |
| [processor.go](source/examples/sdk/aws-deregister/processor.go.txt), `process` and `matches` | Matches a successful deregistration event to an approved target binding. |
| [processor.go](source/examples/sdk/aws-deregister/processor.go.txt), `disable` | Reads CPRa state, checks identity/version, obtains AWS evidence, and submits the conditional mutation. |
| [aws.go](source/examples/sdk/aws-deregister/aws.go.txt), `awsRegistration.absent` and `consume` | Implements AWS registration reads and SQS acknowledgement timing. |
| [demo.go](source/examples/sdk/aws-deregister/demo.go.txt), `runDemo` | Provides a finite exercise without account configuration. |
| [processor_test.go](source/examples/sdk/aws-deregister/processor_test.go.txt) and [review_test.go](source/examples/sdk/aws-deregister/review_test.go.txt) | Exercise duplicate delivery, ambiguous replies, concurrent edits, and input boundaries. |
...
```go
clientConfig, err := exampleutil.ClientConfig(*server, *tokenFile, *allowHTTP)
if err != nil {
    return err
}
...
`cpra.Config` with `BaseURL` and your own `TokenSource`. `exampleutil` is private
...
Copy [mapping.example.json](source/examples/sdk/aws-deregister/mapping.example.json) and replace every fixture value. Read the monitor through the SDK and copy `metadata.id`, `metadata.uid`, and `metadata.resourceVersion` into the mapping. Set `eventsAfter` to the time you approved this binding. Older queued events will be ignored.
...
```sh
AWS_PROFILE=cpra-events go run ./aws-deregister \
...
| `does not provide an available CPRa v2 API` | Check the server version, readiness, authorization, and network path. The read-only v1 server cannot run this configured workflow. |
...
Start by changing `metadata.id` in [monitor.fixture.json](source/examples/sdk/aws-deregister/monitor.fixture.json)
...
<!-- Generated from examples/sdk/aws-deregister/README.md by scripts/sdk/sync_guides.py. -->
