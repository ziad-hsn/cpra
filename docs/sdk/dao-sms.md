...
Follow the parent [SDK examples setup](index.md) to select the Go workspace.
...
| --- | --- |
| [resources.go](source/examples/sdk/dao-sms/resources.go.txt), `resources` | Constructs JobTypes, external drivers, endpoint/group references, and the monitor. |
| [resources.go](source/examples/sdk/dao-sms/resources.go.txt), `register` | Freezes the resource collection, validates references, applies it, and waits using the returned operation ID. |
| [main.go](source/examples/sdk/dao-sms/main.go.txt), `registry` | Binds each JobType/version/category to a compiled handler. |
| [main.go](source/examples/sdk/dao-sms/main.go.txt), `localCredentials` and `run` | Resolves worker-owned credentials and constructs the runner with its encrypted journal. |
| [handlers.go](source/examples/sdk/dao-sms/handlers.go.txt), `daoHealth` and `internalSMS` | Validates assignments, invokes the designated provider, and returns an observed outcome. |
| [handlers.go](source/examples/sdk/dao-sms/handlers.go.txt), `rpcCall` and `providerClient` | Applies HTTP deadlines, response limits, redirect policy, and RPC envelope checks. |
| [demo.go](source/examples/sdk/dao-sms/demo.go.txt), `demo` | Runs the full local lesson and deliberately loses an outcome receipt. |
| [handlers_test.go](source/examples/sdk/dao-sms/handlers_test.go.txt) and [review_test.go](source/examples/sdk/dao-sms/review_test.go.txt) | Verifies provider responses, resource references, credential boundaries, and encrypted outbox delivery behavior. |
...
Start with `daoHealth` in [handlers.go](source/examples/sdk/dao-sms/handlers.go.txt). Each assignment gets one
...
[resources.go](source/examples/sdk/dao-sms/resources.go.txt) builds one collection with these stable IDs:
...
Copy [worker-config.example.json](source/examples/sdk/dao-sms/worker-config.example.json) to a private file
...
[worker storage requirements](source/sdk/go/worker/README.md.txt) for Windows ACLs
...
module. The [SDK guide](index.md) explains module imports and
the full API, while the [worker guide](source/sdk/go/worker/README.md.txt) documents
runner ownership, storage limits, shutdown, and publication status.

<!-- Generated from examples/sdk/dao-sms/README.md by scripts/sdk/sync_guides.py. -->
