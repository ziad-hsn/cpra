...
Read [demo.go](source/examples/sdk/elb-deregistration/demo.go.txt) first. It constructs a client, creates one monitor, and feeds an event to the same processor that configured SQS mode uses. Then read [processor.go](source/examples/sdk/elb-deregistration/processor.go.txt), where the decision to disable is made.
...
| --- | --- |
| [main.go](source/examples/sdk/elb-deregistration/main.go.txt), `run` | Reads flags and mapping, constructs the CPRa client, verifies AWS account identity, and chooses SQS or one-event input. |
| [events.go](source/examples/sdk/elb-deregistration/events.go.txt), `decodeEvent` and `event.validate` | Decodes and validates the event before it can select a monitor. |
| [processor.go](source/examples/sdk/elb-deregistration/processor.go.txt), `process` and `matches` | Matches a successful deregistration event to an approved target binding. |
| [processor.go](source/examples/sdk/elb-deregistration/processor.go.txt), `disable` | Reads CPRa state, checks identity/version, obtains AWS evidence, and submits the conditional mutation. |
| [aws.go](source/examples/sdk/elb-deregistration/aws.go.txt), `awsRegistration.absent` and `consume` | Implements AWS registration reads and SQS acknowledgement timing. |
| [demo.go](source/examples/sdk/elb-deregistration/demo.go.txt), `runDemo` | Provides a finite exercise without account configuration. |
| [processor_test.go](source/examples/sdk/elb-deregistration/processor_test.go.txt) and [review_test.go](source/examples/sdk/elb-deregistration/review_test.go.txt) | Exercise duplicate delivery, ambiguous replies, concurrent edits, and input boundaries. |
...
Copy [mapping.example.json](source/examples/sdk/elb-deregistration/mapping.example.json) and replace every fixture value. Read the monitor through the SDK and copy `metadata.id`, `metadata.uid`, and `metadata.resourceVersion` into the mapping. Set `eventsAfter` to the time you approved this binding. Older queued events will be ignored.
...
Start by changing `metadata.id` in [monitor.fixture.json](source/examples/sdk/elb-deregistration/monitor.fixture.json)
...
<!-- Generated from examples/sdk/elb-deregistration/README.md by scripts/sdk/sync_guides.py. -->
