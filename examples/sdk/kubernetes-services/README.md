...
Prepare the example module using the [examples setup guide](../README.md), then
...
| --- | --- | --- |
| [main.go](main.go) | `command` | Validates flags, loads Kubernetes credentials, constructs both clients, and handles cancellation |
| [discovery.go](discovery.go) | `desired`, `identity` | Converts a Service into typed monitors with stable identities and ownership labels |
...
| [demo.go](demo.go) | `runDemo` | Runs the same discovery path against local Kubernetes and CPRa HTTP fixtures |
| [discovery_test.go](discovery_test.go), [review_test.go](review_test.go) | `Test...` functions | Check target generation, version preconditions, control preservation, and recovery from interrupted watches |
...
The [setup guide](../README.md) explains how to build against the current
...
[RBAC manifest](rbac.yaml) grants those permissions in `payments`. Change every
