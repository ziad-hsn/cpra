...
// ParallelEntityCreator dispatches batches through workers. World mutation is
// serialized because the entity store does not support concurrent writes.
...
func (c *ParallelEntityCreator) processBatch(batch MonitorBatch) error {
	...
	// Set notification context for resolving notify_group references.
	c.entityManager.Endpoints = batch.Endpoints
	c.entityManager.NotificationGroups = batch.NotificationGroups
