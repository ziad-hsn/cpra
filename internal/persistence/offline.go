...
// Raft, expiring history, or invoking a provider. Validation replays committed
// commands into a private in-memory state machine without writing their results.
...
func LockOffline(dir string) (_ *OfflineLock, err error) {
	...
	if (identity.Version != FormatVersion && identity.Version != restoredIdentityVersion) || !identity.Initialized {
	...
	marker, err := readRestoreMarker(dir, identity)
	if err != nil {
		return nil, err
	}
	...
	err = db.View(func(tx *bolt.Tx) error {
		...
		return logs.ForEach(func(_, value []byte) error {
			...
			if _, e := decodeEnvelope(entry.Data); e != nil {
				return fmt.Errorf("invalid command at log index %d: %w", entry.Index, e)
			}
			return nil
		})
	})
	...
	for day, active := range catalog.Segments {
		...
		err = errors.Join(checkOfflineDB(h), validateHistoryOperations(h), h.Close())
		if err != nil {
			return nil, err
		}
	}
	...
	var newest *image
	var newestCollections *collectionLedger
	// Stream collection snapshot rows into an independent private scratch
	// materialization. Validation never rewrites the stopped source directory.
	validationDir, err := os.MkdirTemp("", "cpra-collection-validation-")
	if err != nil {
		return nil, err
	}
	defer os.RemoveAll(validationDir)
	defer func() {
		if newestCollections != nil {
			_ = newestCollections.Close()
		}
	}()
	var newestIndex uint64
	for _, meta := range list {
		_, reader, e := snapshots.Open(meta.ID)
		if e != nil {
			return nil, e
		}
		state, collections, e := decodeSnapshot(reader, validationDir)
		...
		if newest == nil {
			// Own the open generation before metadata validation can return.
			// Windows cannot remove a temporary directory with its DB mapped.
			newestCollections = collections
		}
		if catalog.Index < state.Index {
			return nil, fmt.Errorf("history catalog precedes snapshot; restore a complete backup")
		}
		if state.Index > meta.Index {
			return nil, fmt.Errorf("application snapshot exceeds Raft snapshot position")
		}
		if newest == nil {
			// FileSnapshotStore lists completed snapshots newest first, as
			// consumed by Raft restoration. Older snapshots remain validated.
			newest, newestIndex = &state, meta.Index
		}
	}
	if err = validateOfflineBootstrapWithCollections(db, newest, newestIndex, catalog.Index, marker, newestCollections, validationDir); err != nil {
		return nil, err
	}
	return l, nil
}
