...
func TestPipeline_GzipLoading(t *testing.T) {
	world := ecs.NewWorld()
	em := entities.NewEntityManager(&world)
	...
	path := filepath.Join("..", "controller", "testdata", "test_monitors.yaml.gz")
	stats, err := pipeline.Load(ctx, path)
	if err != nil {
		t.Fatalf("Load failed: %v", err)
	}

	if stats.TotalMonitors != 100 {
		t.Fatalf("TotalMonitors = %d, want 100", stats.TotalMonitors)
	}
	if stats.EntitiesCreated != 100 {
		t.Fatalf("EntitiesCreated = %d, want 100", stats.EntitiesCreated)
	}
}

func TestPipeline_StreamingModeBounded(t *testing.T) {
	if testing.Short() {
		t.Skip("skip streaming bound test in short mode")
	}
	...
	path := filepath.Join("..", "controller", "testdata", "test_monitors_100.yaml")
	stats, err := pipeline.Load(ctx, path)
	if err != nil {
		t.Fatalf("Load failed: %v", err)
	}
	...
	if stats.CreationRate <= 0 {
		t.Fatalf("expected creation rate to be recorded")
	}
}
