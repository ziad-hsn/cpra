// Package preflight validates monitor input without constructing jobs, accessing
...
func Manifest(ctx context.Context, path string, allowEmpty bool) error {
	...
	defer cancel()
	config := streaming.ParseConfig{BatchSize: 1000, MaxMemory: 1 << 30}
	var batches <-chan streaming.MonitorBatch
	var errs <-chan error
	if strings.HasSuffix(strings.TrimSuffix(strings.ToLower(path), ".gz"), ".json") {
		p, err := streaming.NewStreamingJsonParser(path, config)
		if err != nil {
			return err
		}
		batches, errs = p.ParseBatches(ctx, nil)
	} else {
		p, err := streaming.NewStreamingYamlParser(path, config)
		if err != nil {
			return err
		}
		batches, errs = p.ParseBatches(ctx, nil)
