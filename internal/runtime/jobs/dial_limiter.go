...
// DefaultDialLimiterConfig returns sensible defaults for 1M+ monitors.
// At 1M monitors with 40s intervals = 25k checks/sec normal load.
...
func DefaultDialLimiterConfig() DialLimiterConfig {
	return DialLimiterConfig{
		MaxConcurrentDials: 16384, // Caps in-flight dials (doubled for 1M+ monitors)
		DialsPerSecond:     35000, // Prevents goroutine pile-up (25k normal + 40% headroom)
		BurstSize:          4000,  // Limits initial thundering herd burst (doubled)
...
var (
	...
	globalDialLimiterMu sync.RWMutex
)

// initDefaultDialLimiter initializes the global dial limiter with default config.
// Thread-safe and idempotent.
func initDefaultDialLimiter() *DialLimiter {
	globalDialLimiterMu.Lock()
	defer globalDialLimiterMu.Unlock()
	if globalDialLimiter == nil {
...
// For new code, prefer injecting a DialLimiter instance directly.
func GetDialLimiter() *DialLimiter {
	globalDialLimiterMu.RLock()
	dl := globalDialLimiter
	globalDialLimiterMu.RUnlock()
	if dl != nil {
		return dl
	}
	return initDefaultDialLimiter()
}
...
// If the limiter is already initialized, this is a no-op.
...
// SetGlobalDialLimiter replaces the global dial limiter.
// This is primarily for testing and dependency injection.
// Call this at startup before any network operations begin.
//
// Example usage in tests:
//
//	func TestMyJob(t *testing.T) {
//	    limiter := NewDialLimiter(DialLimiterConfig{MaxConcurrentDials: 10})
//	    SetGlobalDialLimiter(limiter)
//	    defer SetGlobalDialLimiter(nil) // Reset after test
//	    // ... run tests
//	}
func SetGlobalDialLimiter(dl *DialLimiter) {
	globalDialLimiterMu.Lock()
	defer globalDialLimiterMu.Unlock()
	globalDialLimiter = dl
}

// ResetGlobalDialLimiter resets the global dial limiter to nil.
// This forces re-initialization on next GetDialLimiter() call.
// Primarily for testing.
func ResetGlobalDialLimiter() {
	globalDialLimiterMu.Lock()
	defer globalDialLimiterMu.Unlock()
	globalDialLimiter = nil
}
