...
// TestCreateCodeJob_NotificationDrivers tests creating unified notification jobs for all driver types
func TestCreateCodeJob_NotificationDrivers(t *testing.T) {
	t.Parallel()
	testCases := []struct {
		name       string
		notify     string
		...
		{"pagerduty", "pagerduty", "critical-service", "red"},
		{"slack", "slack", "web-app", "yellow"},
		{"email", "email", "database", "red"},
		{"webhook", "webhook", "microservice", "green"},
	}
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			config := schema.CodeConfig{Notify: tc.notify, Dispatch: true}
			job, err := CreateCodeJob(tc.monitor, config, ecs.Entity{}, tc.color)
			if err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
			notificationJob, ok := job.(*CodeNotificationJob)
			if !ok {
				t.Fatalf("expected *CodeNotificationJob, got %T", job)
			}
			if notificationJob.DriverName != tc.notify {
				t.Errorf("DriverName = %q, want %q", notificationJob.DriverName, tc.notify)
			}
			if notificationJob.Monitor != tc.monitor {
				t.Errorf("Monitor = %q, want %q", notificationJob.Monitor, tc.monitor)
			}
			if notificationJob.Color != tc.color {
				t.Errorf("Color = %q, want %q", notificationJob.Color, tc.color)
			}
			ReleaseCodeJob(job)
		})
	}
