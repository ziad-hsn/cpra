...
var (
	...
	globalLogManagerMu sync.RWMutex
)

// initDefaultLogManager initializes the global log manager with default config.
// Thread-safe and idempotent.
func initDefaultLogManager() *LogManager {
	globalLogManagerMu.Lock()
	defer globalLogManagerMu.Unlock()
	if globalLogManager == nil {
		globalLogManager = NewLogManager(4096) // Default buffer size
	}
	return globalLogManager
}

// GetLogManager returns the global log manager, creating it if needed.
// For new code, prefer injecting a LogManager instance directly.
func GetLogManager() *LogManager {
	globalLogManagerMu.RLock()
	lm := globalLogManager
	globalLogManagerMu.RUnlock()
	if lm != nil {
		return lm
	}
	return initDefaultLogManager()
}

// SetGlobalLogManager replaces the global log manager.
// This is primarily for testing and dependency injection.
// Call this at startup before any logging operations begin.
//
// Example usage in tests:
//
//	func TestMyJob(t *testing.T) {
//	    lm := NewLogManager(100)
//	    SetGlobalLogManager(lm)
//	    defer func() {
//	        lm.Shutdown()
//	        SetGlobalLogManager(nil)
//	    }()
//	    // ... run tests
//	}
...
// ResetGlobalLogManager resets the global log manager to nil.
// This forces re-initialization on next GetLogManager() call.
// Primarily for testing.
func ResetGlobalLogManager() {
	globalLogManagerMu.Lock()
	defer globalLogManagerMu.Unlock()
	globalLogManager = nil
}

// NewLogManager creates a new LogManager with the given buffer size.
