...
// TestPoolGetPut_CodeNotificationJob tests unified notification job pool
func TestPoolGetPut_CodeNotificationJob(t *testing.T) {
	t.Parallel()
	job1 := getCodeNotificationJob()
	job1.Monitor = "critical-service"
	job1.Color = "red"
	job1.Message = "Service is down"
	job1.DriverName = "slack"

	ReleaseCodeJob(job1)

	job2 := getCodeNotificationJob()
	if job2.Monitor != "" {
		t.Errorf("Monitor should be reset, got %q", job2.Monitor)
	}
	if job2.Message != "" {
	...
	if job2.DriverName != "" {
		t.Errorf("DriverName should be reset, got %q", job2.DriverName)
	}
...
// ============================================================================
// JobPoolManager TESTS
// ============================================================================

// TestNewJobPoolManager tests that JobPoolManager is created with all pools
func TestNewJobPoolManager(t *testing.T) {
	t.Parallel()
	mgr := NewJobPoolManager()
	if mgr == nil {
		t.Fatal("expected non-nil JobPoolManager")
	}

	// Verify all expected pools exist
	expectedPools := []string{
		"pulse_http", "pulse_tcp", "pulse_icmp",
		"intervention_docker", "intervention_docker_stop", "intervention_docker_start",
		"intervention_docker_kill", "intervention_docker_pause", "intervention_docker_unpause",
		"intervention_docker_scale",
		"code_log", "code_notification",
	}

	for _, name := range expectedPools {
		if _, ok := mgr.pools[name]; !ok {
			t.Errorf("expected pool %q to exist", name)
		}
	}
}

// TestJobPoolManager_GetMethods tests all Get methods
func TestJobPoolManager_GetMethods(t *testing.T) {
	t.Parallel()
	mgr := NewJobPoolManager()

	// Test all pulse getters
	if job := mgr.GetPulseHTTPJob(); job == nil {
		t.Error("GetPulseHTTPJob returned nil")
	}
	if job := mgr.GetPulseTCPJob(); job == nil {
		t.Error("GetPulseTCPJob returned nil")
	}
	if job := mgr.GetPulseICMPJob(); job == nil {
		t.Error("GetPulseICMPJob returned nil")
	}

	// Test all intervention getters
	if job := mgr.GetInterventionDockerJob(); job == nil {
		t.Error("GetInterventionDockerJob returned nil")
	}
	if job := mgr.GetInterventionDockerStopJob(); job == nil {
		t.Error("GetInterventionDockerStopJob returned nil")
	}
	if job := mgr.GetInterventionDockerStartJob(); job == nil {
		t.Error("GetInterventionDockerStartJob returned nil")
	}
	if job := mgr.GetInterventionDockerKillJob(); job == nil {
		t.Error("GetInterventionDockerKillJob returned nil")
	}
	if job := mgr.GetInterventionDockerPauseJob(); job == nil {
		t.Error("GetInterventionDockerPauseJob returned nil")
	}
	if job := mgr.GetInterventionDockerUnpauseJob(); job == nil {
		t.Error("GetInterventionDockerUnpauseJob returned nil")
	}
	if job := mgr.GetInterventionDockerScaleJob(); job == nil {
		t.Error("GetInterventionDockerScaleJob returned nil")
	}

	// Test all code getters
	if job := mgr.GetCodeLogJob(); job == nil {
		t.Error("GetCodeLogJob returned nil")
	}
	if job := mgr.GetCodeNotificationJob(); job == nil {
		t.Error("GetCodeNotificationJob returned nil")
	}
}

// TestJobPoolManager_ReleasePulseJob tests releasing pulse jobs
func TestJobPoolManager_ReleasePulseJob(t *testing.T) {
	t.Parallel()
	mgr := NewJobPoolManager()

	// Test HTTP job
	httpJob := mgr.GetPulseHTTPJob()
	httpJob.URL = "http://test.com"
	mgr.ReleasePulseJob(httpJob)
	httpJob2 := mgr.GetPulseHTTPJob()
	if httpJob2.URL != "" {
		t.Errorf("URL should be reset, got %q", httpJob2.URL)
	}

	// Test TCP job
	tcpJob := mgr.GetPulseTCPJob()
	tcpJob.Host = "test.com"
	mgr.ReleasePulseJob(tcpJob)
	tcpJob2 := mgr.GetPulseTCPJob()
	if tcpJob2.Host != "" {
		t.Errorf("Host should be reset, got %q", tcpJob2.Host)
	}

	// Test ICMP job
	icmpJob := mgr.GetPulseICMPJob()
	icmpJob.Host = "ping.test"
	mgr.ReleasePulseJob(icmpJob)
	icmpJob2 := mgr.GetPulseICMPJob()
	if icmpJob2.Host != "" {
		t.Errorf("Host should be reset, got %q", icmpJob2.Host)
	}
}

// TestJobPoolManager_ReleaseInterventionJob tests releasing intervention jobs
func TestJobPoolManager_ReleaseInterventionJob(t *testing.T) {
	t.Parallel()
	mgr := NewJobPoolManager()

	// Test Docker job
	dockerJob := mgr.GetInterventionDockerJob()
	dockerJob.Container = "test-container"
	mgr.ReleaseInterventionJob(dockerJob)
	dockerJob2 := mgr.GetInterventionDockerJob()
	if dockerJob2.Container != "" {
		t.Errorf("Container should be reset, got %q", dockerJob2.Container)
	}
}

// TestJobPoolManager_ReleaseCodeJob tests releasing code jobs
func TestJobPoolManager_ReleaseCodeJob(t *testing.T) {
	t.Parallel()
	mgr := NewJobPoolManager()

	// Test Log job
	logJob := mgr.GetCodeLogJob()
	logJob.Monitor = "test-monitor"
	mgr.ReleaseCodeJob(logJob)
	logJob2 := mgr.GetCodeLogJob()
	if logJob2.Monitor != "" {
		t.Errorf("Monitor should be reset, got %q", logJob2.Monitor)
	}

	// Test Notification job
	notifJob := mgr.GetCodeNotificationJob()
	notifJob.Monitor = "test-monitor"
	notifJob.DriverName = "slack"
	mgr.ReleaseCodeJob(notifJob)
	notifJob2 := mgr.GetCodeNotificationJob()
	if notifJob2.Monitor != "" {
		t.Errorf("Monitor should be reset, got %q", notifJob2.Monitor)
	}
	if notifJob2.DriverName != "" {
		t.Errorf("DriverName should be reset, got %q", notifJob2.DriverName)
	}
}

// TestDefaultPoolManager tests the global default pool manager
func TestDefaultPoolManager(t *testing.T) {
	t.Parallel()
	mgr := DefaultPoolManager()
	if mgr == nil {
		t.Fatal("DefaultPoolManager() returned nil")
	}

	// Verify it's the same instance
	mgr2 := DefaultPoolManager()
	if mgr != mgr2 {
		t.Error("DefaultPoolManager should return the same instance")
	}
}

// TestJobPoolManager_IndependentInstances tests that different managers are independent
func TestJobPoolManager_IndependentInstances(t *testing.T) {
	t.Parallel()
	mgr1 := NewJobPoolManager()
	mgr2 := NewJobPoolManager()

	// Get jobs from both
	job1 := mgr1.GetPulseHTTPJob()
	job1.URL = "http://manager1.com"

	job2 := mgr2.GetPulseHTTPJob()
	job2.URL = "http://manager2.com"

	// Release to respective managers
	mgr1.ReleasePulseJob(job1)
	mgr2.ReleasePulseJob(job2)

	// Each should get clean jobs back
	newJob1 := mgr1.GetPulseHTTPJob()
	newJob2 := mgr2.GetPulseHTTPJob()

	if newJob1.URL != "" {
		t.Errorf("mgr1 job URL should be reset, got %q", newJob1.URL)
	}
	if newJob2.URL != "" {
		t.Errorf("mgr2 job URL should be reset, got %q", newJob2.URL)
	}
}
