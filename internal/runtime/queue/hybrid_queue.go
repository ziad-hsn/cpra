...
// Package-level constants that reference the centralized constants package.
// This allows for easy updates and documentation in one place.
const (
	defaultRingCapacity     = constants.DefaultRingCapacity
	defaultOverflowCapacity = constants.DefaultOverflowCapacity
	defaultSoftWatermark    = constants.SoftWatermark
	defaultHardWatermark    = constants.HardWatermark
)
...
// cacheLinePad is used to separate atomics into different cache lines
// to prevent false sharing on multi-core systems. A cache line is typically 64 bytes.
...
//
// PERFORMANCE NOTE: Atomic fields are grouped by access pattern (producer vs consumer)
// with cache line padding to prevent false sharing on high-core systems.
type HybridQueue struct {
	// === Non-atomic fields (accessed during initialization) ===
	...
	// === Producer hot path atomics (own cache line) ===
	_padProd      cacheLinePad
	ringDepth     atomic.Int64 // Updated on every enqueue/dequeue
	overflowDepth atomic.Int64 // Updated on overflow operations
	enqueuedCount atomic.Int64 // Updated on every enqueue

	// === Consumer hot path atomics (own cache line) ===
	_padCons       cacheLinePad
	dequeuedCount  atomic.Int64 // Updated on every dequeue
	totalQueueWait atomic.Int64 // Updated on every dequeue
	maxQueueWait   atomic.Int64 // Updated via CAS on dequeue

	// === Cold path atomics (rarely updated, can share) ===
	_padCold            cacheLinePad
	...
	// === Mutex-protected overflow (own cache line to avoid contention with atomics) ===
	_padMu cacheLinePad
	mu     sync.Mutex
}
...
func (q *HybridQueue) notify() {
	...
	// Channel has buffer of 1, so if a signal is already pending (not yet consumed),
	// this is a no-op. This naturally coalesces multiple rapid enqueues into one wake-up.
	select {
	case q.signal <- struct{}{}:
	default:
		// Channel already has a pending signal, consumer will see it
	}
}

// AckSignal is a no-op for compatibility. Signal coalescing is handled by the channel buffer.
func (q *HybridQueue) AckSignal() {
	// No-op: signal coalescing via buffered channel doesn't require acknowledgment
}
...
func (q *HybridQueue) tryDequeueOverflow() (jobs.Job, bool) {
	...
	job := q.overflow[0]
	// Efficient removal without retaining references to dequeued job
	if len(q.overflow) == 1 {
		...
		// Periodically shrink capacity to prevent long-lived large slices
		if cap(q.overflow) > len(q.overflow)*4 && cap(q.overflow) > 1000 {
			newSlice := make([]jobs.Job, len(q.overflow), len(q.overflow)*2)
			copy(newSlice, q.overflow)
			q.overflow = newSlice
		}
	}
...
func normalizeHybridConfig(cfg HybridQueueConfig) HybridQueueConfig {
	...
	if cfg.OverflowCapacity == 0 {
		...
		if cfg.SoftWatermark <= 0 {
			...
			cfg.SoftWatermark = max(0.0, min(1.0, cfg.SoftWatermark))
		}
		if cfg.HardWatermark <= 0 {
			cfg.HardWatermark = defaults.HardWatermark
		} else {
			cfg.HardWatermark = max(0.0, min(1.0, cfg.HardWatermark))
		}
