...
func NewWorkivaQueue(capacity int) Queue {
	...
	q.rollingWait = NewRollingWait(defaultRollingWaitSize)
	q.emaWait = NewEMAWait(0.2)
	q.bucketedWait = NewTimeBucketedWait(time.Minute, 10)
	return q
}
	...
	if len(out) > 0 {
		...
		for _, j := range out {
			...
			if !enqueueTime.IsZero() {
				...
				if w > 0 {
					if q.rollingWait != nil {
						q.rollingWait.Record(time.Duration(w))
					}
					if q.emaWait != nil {
						q.emaWait.Record(time.Duration(w))
					}
					if q.bucketedWait != nil {
						q.bucketedWait.Record(time.Duration(w))
					}
				}
			}
		}
