...
func TestTwilioAcceptanceBoundaryNeverClaimsDelivery(t *testing.T) {
	for _, status := range []int{http.StatusCreated, http.StatusUnauthorized} {
		t.Run(http.StatusText(status), func(t *testing.T) {
			...
			defer server.Close()
			cfg := Config{
				Manifest: schema.Manifest{Monitors: []schema.Monitor{{ID: "twilio", Name: "twilio", Codes: schema.Codes{"red": {
					Notify: "twilio", Dispatch: true, Config: &schema.CodeNotificationTwilio{URL: server.URL + "/2010-04-01/Accounts/ACfixture/Messages.json", AccountSID: "ACfixture", AuthToken: "SECRET_TEST_TOKEN", From: "+15005550006", To: "+15005550006"},
				}}}}},
